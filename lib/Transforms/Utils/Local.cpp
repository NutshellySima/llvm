...
void llvm::MergeBasicBlockIntoOnlyPred(BasicBlock *DestBB,
  ...
  if (DTU) {
    ...
    if (DTU->isEager()) {
      ...
      // Update the PostDomTree
      if (DTU->hasPostDomTree()) {
        PostDominatorTree &PDT = DTU->getPostDomTree();
        if (DomTreeNode *PredDTN = PDT.getNode(PredBB)) {
          DomTreeNode *DestDTN = PDT.getNode(DestBB);
          SmallVector<DomTreeNode *, 8> Children(PredDTN->begin(),
                                                 PredDTN->end());
          for (DomTreeNode *DI : Children)
            PDT.changeImmediateDominator(DI, DestDTN);
        }
      }

      // Delete PredBB under Eager UpdateStrategy.
      DTU->deleteBB(PredBB);
    }

    // Lazy UpdateStrategy
    else {
      DTU->deleteBB(PredBB); // Delete PredBB under Lazy UpdateStrategy.
...
bool llvm::TryToSimplifyUncondBranchFromEmptyBlock(BasicBlock *BB,
  ...
  // Delete the edge between BB and Succ.
  if (BB->getTerminator()) {
  ...
  if (DTU) {
    DTU->applyUpdates(Updates, /*ForceRemoveDuplicates*/ true);
    DTU->deleteBB(BB); // Deferred deletion of the old basic block.
  } else {
      ...
      if (auto *II = dyn_cast<IntrinsicInst>(&I)) {
        if (II->getIntrinsicID() == Intrinsic::assume) {
          if (match(II->getArgOperand(0), m_CombineOr(m_Zero(), m_Undef()))) {
            ...
            changeToUnreachable(II, false, false, DTU);
            Changed = true;
            break;
          }
        }
